...
export default function EditarPetPage({
  ...
  const { pets, atualizarPet } = useAppStore();
  const pet = pets.find((p) => p.id === petId);

  // Redirecionar se pet não encontrado
  if (!pet) {
    router.replace("/");
    return null;
  }
  ...
  function salvar() {
    ...
    atualizarPet(petId, {
      ...
      vacina: form.temVacina && form.vacinaData
    ...
    <div className="flex flex-col min-h-screen bg-white">
      <header className="flex items-center gap-3 px-5 pt-14 pb-4">
        ...
        <div className="flex-1">
          <h1 className="text-lg font-semibold text-navy">Editar Pet</h1>
          <p className="text-xs text-gray-400">Passo {step}/{STEPS.length}</p>
        </div>
      </header>
