...
export default function EditarPetPage({
  ...
  const getPet = useAppStore((s) => s.getPet);
  ...
  useEffect(() => {
    if (pet) {
      setForm({
        ...
        tipoPet: pet.tipoPet,
        temVacina: !!pet.vacina?.valida,
        vacinaData: pet.vacina?.data ?? "",
        vacinaNome: pet.vacina?.nomeComercial ?? "",
        temSorologia: pet.sorologia?.status === "OK",
  ...
  function salvar() {
    ...
    router.replace(`/passaporte/${petId}`);
  }
    ...
    <div className="flex flex-col min-h-screen">
      <header className="flex items-center gap-3 px-5 pt-14 pb-4">
        ...
        <div className="flex-1">
          <h1 className="text-lg font-semibold">Editar Pet</h1>
        ...
        <div className="flex justify-between mt-1.5">
          {STEPS.map((s) => (
            <span
              key={s.id}
              className={`text-[10px] ${
                s.id === step ? "text-teal" : "text-gray-400"
              }`}
            >
              {s.label}
            </span>
      ...
      <div className="flex-1 px-5 overflow-hidden">
        <AnimatePresence mode="wait" custom={direction}>
          ...
          >
            {step === 1 && (
              <StepIdentificacao form={form} update={update} />
            )}
    ...
    <div className="space-y-5">
      ...
      <div>
        ...
        <p className="text-xs text-gray-400 mt-1">
          Obrigatório para viagens internacionais. Deixe vazio se ainda não
          implantou.
        </p>
      </div>
        ...
        <div>
          ...
          {form.tipoPet === "CAO_GUIA" && (
            <p className="text-xs text-emerald-600 mt-1.5 ml-1">
              🦮 Cão-guia embarca obrigatoriamente em cabine, gratuito (Lei
              11.126/2005)
            </p>
          )}
        </div>
    ...
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-gray-500">
          Obrigatória para todos os destinos
        </p>
      </div>
      ...
      {form.temVacina && (
        ...
        >
          ...
          <Field
            ...
            placeholder="Ex: Rabisin, Defensor"
          />
        </motion.div>
      )}
    </div>
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-gray-500">
          Obrigatória para Europa e Japão
        </p>
      </div>
        ...
        <div className="flex items-start gap-3 bg-gray-100/50 border border-gray-200 rounded-xl px-4 py-4">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">
              A sorologia antirrábica só pode ser realizada após a vacinação.
              Volte ao passo anterior e registre a vacina primeiro.
            </p>
          </div>
        </div>
      ) : (
