...
import { Eye, EyeOff, Mail, Lock, User, AlertCircle, CheckCircle2, PawPrint } from "lucide-react";
import { motion } from "framer-motion";
import { signUpWithEmail, signInWithGoogle, signInWithApple } from "@/services/auth-service";
...
export default function CadastroPage() {
  ...
  const senhaValida = REQUISITOS_SENHA.every((r) => r.test(senha));
  ...
  async function handleCadastro(e: React.FormEvent) {
    ...
    const { error, data } = await signUpWithEmail(email, senha, nome);
    ...
    // Se email de confirmação foi enviado (email not confirmed)
    if (data.user && !data.session) {
      router.push(`/auth/verificar-email?email=${encodeURIComponent(email)}`);
      return;
    }

    // Login instantâneo (confirmação desabilitada no Supabase)
  ...
  async function handleApple() {
    ...
    if (error) {
      setErro("Não foi possível iniciar o cadastro com Apple.");
      setLoadingProvider(null);
    }
  }

  return (
    <div className="min-h-screen flex flex-col justify-between px-5 pt-12 pb-8">
      {/* Header */}
      <motion.div
