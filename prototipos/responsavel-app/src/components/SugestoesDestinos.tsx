...
function DestinoCard({ sugestao: s }: { sugestao: SugestaoDestino }) {
  return (
    <div className="flex-shrink-0 w-48 bg-gray-800 rounded-2xl overflow-hidden border border-gray-700/50 hover:border-sky-700/50 transition-colors">
      {/* Imagem / emoji cover */}
      <div className="h-20 bg-gradient-to-br from-gray-700 to-gray-900 flex items-center justify-center text-4xl relative">
        {s.imagemEmoji}
      ...
      <div className="p-3">
        <p className="text-white text-xs font-semibold leading-tight line-clamp-1">{s.nome}</p>
        ...
        {s.destinoCompliance && (
          <p className="text-[10px] text-sky-500 mt-2">Ver documentação →</p>
        )}
      </div>
    </div>
  );
